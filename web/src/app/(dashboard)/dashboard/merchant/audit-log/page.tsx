...
import { auditApi, merchantApi, type AuditLogResponse, type AuditPagedResult } from '@/lib/api';
import {
  Search, Filter, ChevronLeft, ChevronRight,
  ShieldCheck, AlertTriangle, XCircle, Building2, RefreshCw,
} from 'lucide-react';
...
export default function MerchantAuditLogPage() {
  ...
  // Load merchant API key on mount
  useEffect(() => {
    if (!user) return;
    merchantApi.getByUser(user.id)
      .then(res => {
        const m = res.data?.[0];
        if (m) { setApiKey(m.apiKey); setMerchantName(m.name || 'Merchant'); }
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await auditApi.getMerchantLogs(apiKey, {
      ...
      toast.error(msg || 'Failed to load merchant audit log. Make sure the API server is running.');
    } finally {
      setLoading(false);
    }
  }, [apiKey, search, action, entityType, status, from, to, page]);
  ...
  if (!loading && !apiKey) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-3">
        ...
        <p className="text-sm font-medium">No merchant found for your account.</p>
        <p className="text-xs">
          Go to{' '}
          <a href="/dashboard/merchant" className="text-brand-600 underline">
            Merchant
          </a>{' '}
          to set one up.
        </p>
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-gray-900">Merchant Audit Log</h1>
            {merchantName && (
          ...
          <p className="mt-1 text-sm text-gray-500">
            All activity recorded for your merchant account
          </p>
        </div>
        <button
          onClick={load}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {result.items.map(row => (
                  <MerchantAuditRow key={row.id} row={row} />
                ))}
              </tbody>
            </table>
          </div>
...
function MerchantAuditRow({ row }: { row: AuditLogResponse }) {
